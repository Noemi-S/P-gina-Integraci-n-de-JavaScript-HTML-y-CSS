...
function ClickCalcularMediana(){
    const inputDatos = document.getElementById("InputDatos2").value;
    const DatosOrdenados=Array.from(inputDatos.split(","), Number);
...
function ClickCalcularPromedio(){
    const inputDatos = document.getElementById("InputDatos3").value;
    const DatosOrdenados=Array.from(inputDatos.split(","), Number);
    const Promedio=CalculoMediaAritmetica(DatosOrdenados);
    const prom = document.getElementById("Promedio");
    prom.innerText = "El promedio es:" + Promedio;
}
